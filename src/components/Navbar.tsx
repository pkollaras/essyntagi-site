...
const Navbar = () => {
  ...
  return (
    <header className={cn(
      'fixed top-0 left-0 right-0 z-50 transition-all duration-300 py-2 sm:py-4 px-4 sm:px-6 md:px-10', 
      isScrolled ? 'glass shadow-subtle py-2 sm:py-3' : 'bg-transparent'
    )}>
      <div className="container mx-auto">
        <nav className="flex items-center justify-between">
          {/* Logo */}
          <a href="#" className="flex items-center">
            <img alt="Eprescription Logo" src="/lovable-uploads/cc531653-f938-4549-a811-886f5ff300a6.png" className="h-14 sm:h-20 w-auto bg-transparent" />
          </a>
          ...
          <div className="hidden md:flex items-center space-x-4 lg:space-x-6">
            {navLinks.map(link => (
              <button 
                ...
                className="text-gray-700 hover:text-esyntagi-600 hover-underline font-medium transition text-sm lg:text-base"
              >
                {link.label}
              </button>
            ))}
            ...
            <a 
              ...
              className="bg-blue-600 hover:bg-blue-700 text-white rounded-full px-4 sm:px-6 py-2 inline-flex items-center justify-center text-xs sm:text-sm font-medium transition-colors"
            >
              Ξεκινήστε
            </a>
            ...
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer" 
              className="flex items-center text-esyntagi-600 hover:text-esyntagi-700 font-medium transition text-sm lg:text-base"
            >
              <span>Login</span>
