    ...
    <footer className="bg-gray-50 pt-16 pb-8">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-8 pb-12">
          ...
          {/* Empty div with 2 columns - hidden on mobile */}
              ...
              <ul className="flex flex-col space-y-3">
                {footerMenu.map((item, index) => (
                  <li key={index}>
                    <a 
                      href={item.link} 
                      className="text-gray-600 hover:text-blue-600 transition-colors duration-300 text-sm sm:text-base"
                    >
                      {item.label}
                    </a>
                  </li>
